...
{   
    ...
    if (gc && values.size() > 0)
    {
        ...
        int iddd = evt.GetId();
        if(evt.GetId() == 121)
            gc->SetPen(wxPen(wxSystemSettings::GetAppearance().IsDark() ? wxTheColourDatabase->Find("LIME GREEN") : *wxBLUE, 3));
        else
            gc->SetPen(wxPen(wxSystemSettings::GetAppearance().IsDark() ? wxTheColourDatabase->Find("PURPLE") : *wxBLUE, 3));
