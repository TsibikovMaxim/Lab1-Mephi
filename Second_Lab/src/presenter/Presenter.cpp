...
pair<string, bool> Presenter::find(string findType, string value) {
    ...
    string time;
    bool resBool;
    if (findType.size() == 0 || value.size() == 0 || arr->getLength() == 0)
    ...
    if (findType == "Hash Table") {
        ...
        HashTable<int> hashTable(arrPair.first, arrPair.second);
        start = clock();
        resBool = hashTable.find(value);
        end = clock();
        time = to_string((int)(end - start) / (CLOCKS_PER_SEC / 1000)) + " ms";
        return make_pair(time, resBool);
    }
...
void Presenter::createMat(int size) {
    ...
    for (int i = 0; i < matrix_size; i++)
        for (int j = 0; j < matrix_size; j++)
            if (rand() % 10+1 < 8) // примерно 80% значений будут равны нулю
                matrix[i][j] = 0;
            else
...
string Presenter::processMat() {
    ...
    for (int i = 0; i < matrix_size; i++)
        for (int j = 0; j < matrix_size; j++)
            if(matrix[i][j] != 0)
                hashTable.add(to_string(matrix[i][j]), "Row: " + to_string(i) + " ,Col: " + to_string(j));
...
tuple<vector<double>, vector<double>, vector<double>> Presenter::getPointsForChart() {
    ...
    for(int i = 2; i < 10000; i++) {
        ...
        //2
        HashTable<int> hashTable(arr, i);
        start = clock();
        hashTable.find(to_string(valueToFind));
        end = clock();
        time = (double)(end - start) / (CLOCKS_PER_SEC / 1000);
